# Pomera DM250 Backup & Recovery Toolkit (ポメラ DM250 バックアップ＆復旧ツールキット)

[English](README.md) | [日本語](README.ja.md)

本ツールキットは、**キングジム ポメラ DM250** の内蔵eMMCをPCへバックアップしたり、保存したバックアップイメージから本体を書き戻して復旧するためのツールキットです。本体を分解することなく、**SDカードブートによる USB Mass Storage (UMS) モード** を経由してPCからeMMCの読み書きを行います。
...
>    * USB接続後は時間を空けずに速やかに `backup_emmc.sh` または `restore_emmc.sh` を実行し、確認プロンプト（`yes`）を進めてデータ転送を開始してください（転送が始まれば切断されなくなります）。
...
### ステップ 4-A: 【バックアップ取得】ポメラ eMMC を PC に保存 (`backup_emmc.sh`)

ポメラ側への書き込みを一切行わないリードオンリー（安全）動作で、PC側のストレージへ丸ごとダンプします。
※ `<target_device>` には、ステップ3で確認したポメラのデバイス名（**Linux: `/dev/sdX`**、**macOS: `/dev/rdiskN`**）を指定します。
...
### ステップ 4-B: 【リストア復旧】バックアップをポメラに書き戻し (`restore_emmc.sh`)

...
# PCからの誤書き込み・上書きを物理ブロックする安全モードです（-r, --ro も可）。
./prepare_sdcard.sh

# 【復元時のみ: 書き込み許可 Read-Write モード】
# ポメラ内蔵eMMCへのデータ書き戻し（リストア）を行いたい場合はこちらを指定します（-w, --rw も可）。
...
> * Pomera へバックアップを書き戻す（復元する）時のみ、`--readwrite`（または `--rw`, `-w`）を指定して SD カードを作成してください。
...
4. 画面バックライトが点灯し、最初に U-Boot の自動起動待機カウントダウン（3秒間）が表示されます：
   ```text
   Hit any key to stop autoboot:  3 ... 2 ... 1
   ```
   * *(※ このカウントダウン中にキーボードのキーを押すと U-Boot の対話プロンプト `=>` で止まりますが、通常は何も押さずに待つだけで自動起動します)*
   * 3秒経過すると自動的に画面が切り替わり、モードに応じた案内バナーが表示されて UMS モードが開始されます：
...
5. **USB Type-Cケーブルで Pomera と PC を接続**します。
   * 接続が確立されると、Pomera の画面上にリアルタイムで接続通知が表示されます（作成したSDカードのモードに応じて表示されます）：
   ```text
   >>> [USB] Connected to Host PC (eMMC Ready - READ-ONLY) <<<
   ```
   *(※ `--readwrite` で作成した SD カードの場合は `(eMMC Ready - READ-WRITE)` と表示されます)*
   *(※ USBケーブルを抜くと `>>> [USB] Disconnected (Reboot Pomera to reconnect) <<<` と表示されます。再接続したい場合は Pomera を再起動してください)*
...
> リストア（復元・書き戻し）を行う際は、あらかじめ **`./prepare_sdcard.sh --readwrite`（または `--rw`, `-w`）** で書き込みを許可した SD カードを作成・挿入して Pomera を起動してください。

PC側でリストアスクリプトを実行します（※ `<target_device>` には **Linux: `/dev/sdX`**、**macOS: `/dev/rdiskN`** を指定）：

```bash
# バックアップフォルダを直接指定して復元する場合（推奨）
sudo ./restore_emmc.sh <target_device> ./factory_backup

# 引数省略時（./restore_file/ ディレクトリ内のイメージから復元）
sudo ./restore_emmc.sh <target_device>
```

> [!NOTE]
> `backup_emmc.sh` を第2引数（保存先ディレクトリ）なしで実行した場合のデフォルト保存先は `./backup_file/` です。そのバックアップから復元する場合は、`sudo ./restore_emmc.sh <target_device> ./backup_file` のように明示的に指定するか、ファイルを `./restore_file/` に配置して実行してください。
